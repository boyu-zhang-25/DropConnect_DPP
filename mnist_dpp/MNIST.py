...
# MLP of size 784 - 500 - 500 - 10 (DIVNET, ICLR 2016)
class MLP(nn.Module):
	...
	def __init__(self,
		...
		# DIVNET
		self.w1 = nn.Linear(28 * 28, hidden_size) # hidden_size * 500
		self.w2 = nn.Linear(hidden_size, 500) 
		self.w3 = nn.Linear(500, 10) 
	...
	def forward(self, x):
		...
		# batch_size * (28 * 28) -> batch_size * hidden_size
...
def main():
	...
	if args.procedure == 'training':
		...
		for epoch in range(1, args.epochs + 1):
			...
			test_acc = test(args, model, device, test_loader, criterion)

			# DIVNET trained till Train Acc. >= 99%
			if train_acc >= 99:
				break
			

		if args.save_model:
			name = 'MNIST_' + str(args.probability) + '_batch' + str(args.train_batch_size) + '.pth'
